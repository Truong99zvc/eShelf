...
const Bookmarks = () => {
  ...
  const handleRemove = async (isbn) => {
    setRemoving(isbn);
    try {
      await userService.removeBookmark(isbn);
  ...
  return (
    <div className="mx-4 py-8 md:mx-20 lg:mx-32">
      <div className="mb-6 flex items-center gap-3">
        <Bookmark className="h-8 w-8 text-amber-500" fill="#f59e0b" />
        <h1 className="text-2xl font-bold text-gray-800">Sách Đã Đánh Dấu</h1>
      </div>

      {books.length === 0 ? (
        <div className="rounded-lg border border-gray-200 bg-white py-16 text-center">
          <Bookmark className="mx-auto mb-4 h-16 w-16 text-gray-300" />
          <p className="text-gray-500">Bạn chưa đánh dấu sách nào</p>
          <Link
            to="/"
          ...
          {books.map((book) => (
            ...
            >
              ...
              <button
                ...
                className="absolute right-2 top-2 rounded-full bg-white/90 p-2 text-amber-500 opacity-0 transition-opacity hover:bg-amber-50 group-hover:opacity-100"
                title="Xóa khỏi đánh dấu"
              >
                {removing === book.isbn ? (
